...
func (h *GoKitHandler) Handle(_ context.Context, record slog.Record) error {
	if h.levelLoggers == nil {
		// Handlers constructed manually (ie, without
		// NewGoKitHandler()), won't have level cache yet. Create one.
		if h.logger == nil {
			h.logger = setCaller(defaultGoKitLogger)
		}
		h.levelLoggers = newLevelCache(h.logger)
	}
